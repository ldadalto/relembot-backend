...
- `POST /billing/sync {purchaseTokens: [...]}` consulta `subscriptionsv2`, valida produto, vínculo e validade. Tokens são criptografados com AES-256-GCM. Tokens antigos que o Google retorna como inválidos (404/410) não impedem sincronizar a revogação ou uma compra substituta; conflitos de titularidade e indisponibilidade do provedor continuam bloqueados. Booleanos de assinatura são rejeitados. `GET /account/entitlement` retorna a mesma situação verificada.
